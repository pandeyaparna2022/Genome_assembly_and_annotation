...
#SBATCH --output=/data/users/apandey/genome_and_transcriptome_assembly_and_annotation/analysis/logs/07_meryl_kmercount_analysis_%j.o
#SBATCH --error=/data/users/apandey/genome_and_transcriptome_assembly_and_annotation/analysis/logs/07_meryl_kmercount_analysis_%j.e
...
INPUT_FLYE_UNPOLISHED_ASSEMBLY=${assembly_directory}/analysis/03_genome_assemblies/Flye_genome_assembly/assembly.fasta
INPUT_CANU_UNPOLISHED_ASSEMBLY=${assembly_directory}/analysis/03_genome_assemblies/Canu_genome_assembly/*.contigs.fasta
...
mkdir -p ${OUTPUT_DIR}/Flye_unpolished
cd ${OUTPUT_DIR}/Flye_unpolished
...
output_prefix=Flye_unpolished

apptainer exec \
--bind "${assembly_directory}" \
/software/singularity/containers/Merqury-1.3-1.ubuntu20.sif \
merqury.sh ${MERYL_DIR} ${INPUT_FLYE_UNPOLISHED_ASSEMBLY} ${output_prefix}

# Run Mercury analysis on unpolished canu assembly
# Make a custopm directory for results and go to that directory
mkdir -p ${OUTPUT_DIR}/Canu_unpolished
cd ${OUTPUT_DIR}/Canu_unpolished

# grant read, write, and execute permissions to all users (owner, group, and others) for the assembly file
chmod a+rwx ${INPUT_CANU_UNPOLISHED_ASSEMBLY}
# assign the value "Canu_unpolished" to the variable output_prefix
output_prefix=Canu_unpolished

apptainer exec \
--bind ${assembly_directory} \
/software/singularity/containers/Merqury-1.3-1.ubuntu20.sif \
merqury.sh ${MERYL_DIR} ${INPUT_CANU_UNPOLISHED_ASSEMBLY} ${output_prefix}

# Run Mercury analysis on polished flye assembly
# Make a custopm directory for results and go to that directory
mkdir -p ${OUTPUT_DIR}/Flye_polished
cd ${OUTPUT_DIR}/Flye_polished

# grant read, write, and execute permissions to all users (owner, group, and others) for the assembly file
chmod a+rwx ${INPUT_FLYE_POLISHED_ASSEMBLY}
# assign the value "Flye_polished" to the variable output_prefix
output_prefix=Flye_polished

apptainer exec \
--bind ${assembly_directory} \
/software/singularity/containers/Merqury-1.3-1.ubuntu20.sif \
merqury.sh ${MERYL_DIR} ${INPUT_FLYE_POLISHED_ASSEMBLY} ${output_prefix} 

# Run Mercury analysis on polished canu assembly
# Make a custopm directory for results and go to that directory
mkdir -p ${OUTPUT_DIR}/Canu_polished
cd ${OUTPUT_DIR}/Canu_polished

# grant read, write, and execute permissions to all users (owner, group, and others) for the assembly file
chmod a+rwx ${INPUT_CANU_POLISHED_ASSEMBLY}
# assign the value "Canu_polished" to the variable output_prefix
output_prefix=Canu_polished

apptainer exec \
--bind ${assembly_directory} \
/software/singularity/containers/Merqury-1.3-1.ubuntu20.sif \
merqury.sh ${MERYL_DIR} ${INPUT_CANU_POLISHED_ASSEMBLY} ${output_prefix}
